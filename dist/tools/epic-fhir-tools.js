...
class EpicFHIRTools {
    ...
    getKnownPatientById(patientId) {
        const knownPatientsById = {
    ...
    getMockObservations(patientId, category) {
        ...
        const filteredObs = category ? observations.filter((obs) => obs.category === category) : observations;
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
    ...
    getMockMedications(patientId, status) {
        ...
        const filteredMeds = status ? medications.filter((med) => med.status === status) : medications;
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
    ...
    getMockConditions(patientId, clinicalStatus) {
        ...
        const filteredConditions = clinicalStatus ? conditions.filter((cond) => cond.clinicalStatus === clinicalStatus) : conditions;
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
    ...
    getMockEncounters(patientId, status) {
        ...
        const filteredEncounters = status ? encounters.filter((enc) => enc.status === status) : encounters;
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
